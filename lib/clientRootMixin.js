...
var CLASS_EDITOR = 'vuepress-plugin-demo-block__editor';
...
function webController() {
  ...
  nodes.length && nodes.forEach(function (node) {
    ...
    var editorNode = $(node, CLASS_EDITOR);
    ...
    var myCodeMirror = CodeMirror(editorNode, {
      value: code,
      theme: 'material',
      tabSize: 2,
      lineNumbers: true,
    ...
    myCodeMirror.on('changes', function (codeMir) {
      ...
      var code = codeMir.doc.getValue();
      var detail = getVueDetail(code, config);
      detail.css && injectCss(detail.css);
      vm.$destroy();
      console.log(appNode);
      appNode.removeChild(vm.$el);
      vm = new window.Vue(detail.script).$mount();
      appNode.appendChild(vm.$el); // setTimeout(_ => {
